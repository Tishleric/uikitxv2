"""
Greek Configuration Module for Spot Risk Calculator
...
# Default Greek configuration - optimized for performance
...
# All possible Greeks that can be calculated
ALL_GREEKS = set(DEFAULT_ENABLED_GREEKS.keys()) | {'speed_y'}


class GreekConfiguration:
    """Manages Greek calculation configuration for performance optimization."""
    
    def __init__(self, enabled_greeks: Optional[Dict[str, bool]] = None):
        """
        ...
        logger.info(f"Greek configuration initialized: {len(self.get_enabled_greeks())} Greeks enabled")
        logger.debug(f"Enabled Greeks: {sorted(self.get_enabled_greeks())}")
    
    def _load_from_env(self):
        """Load configuration from environment variables if present."""
        env_greeks = os.environ.get('SPOT_RISK_GREEKS_ENABLED')
        if env_greeks:
            logger.info(f"Loading Greek configuration from environment: {env_greeks}")
            # Parse comma-separated list of Greek names
            enabled_list = [g.strip() for g in env_greeks.split(',') if g.strip()]
            ...
            for greek in enabled_list:
                if greek in self.enabled_greeks:
                    ...
                    logger.warning(f"Unknown Greek in environment config: {greek}")
    
    def _validate_configuration(self):
        """Validate that required Greeks are enabled for positions aggregator."""
        # Check if we need position Greeks (default is yes)
        ...
        if needs_positions:
            ...
            if missing_required:
                logger.warning(f"Enabling required Greeks for positions aggregator: {missing_required}")
    ...
    def is_enabled(self, greek_name: str) -> bool:
        ...
        if greek_name == 'speed_y':
            # speed_y is derived from speed_F
    ...
    def get_api_requested_greeks(self) -> List[str]:
        ...
        Returns:
            List of Greek names that the API should calculate
        """
        api_greeks = []
        enabled = self.get_enabled_greeks()
        
        # The API expects these specific Greek names
        # We only request Greeks that are enabled
        greek_mapping = {
            'delta_F': 'delta_F',
            'delta_y': 'delta_y',
            'gamma_F': 'gamma_F',
