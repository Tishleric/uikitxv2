...
# Import the protocol from components package
from components.core import DataServiceProtocol

# Import Pricing Monkey modules from new locations
...
class ActantDataService(DataServiceProtocol):
    ...
    def __init__(self, db_path: str = "data/output/eod/actant_eod_data.db"):
        """
    ...
    def load_pricing_monkey_data(self) -> bool:
        ...
        """
        try:
            logger.info("Starting PricingMonkey data retrieval")
            
            # Get the extended PM data (includes 3M, 6M, 1Y data)
            pm_data_dict = get_extended_pm_data()
            
            # Validate data structure  
            if not validate_pm_data(pm_data_dict):
                logger.error("PM data validation failed")
                return False
            
            # Process and save to separate table
            pm_df = process_pm_for_separate_table(pm_data_dict)
            
            if pm_df.empty:
                logger.warning("No PM data to save")
                return False
                
            # Save to database
            self._save_pm_to_database(pm_df)
            self._pm_data_loaded = True
            
            logger.info(f"Successfully loaded PM data: {len(pm_df)} rows")
            return True
            
        except PMRetrievalError as e:
            logger.error(f"PM retrieval error: {e}")
            return False
        except Exception as e:
            logger.error(f"Unexpected error loading PM data: {e}")
            return False
