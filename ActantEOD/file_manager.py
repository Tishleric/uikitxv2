...
DEFAULT_JSON_FOLDER = r"Z:\ActantEOD"
FALLBACK_JSON_FOLDER = "ActantEOD"  # Local fallback if Z: drive not accessible
JSON_FILE_PATTERN = "*.json"


def get_json_folder_path() -> Path:
    """
    Get the JSON folder path, with fallback to local directory if shared folder unavailable.
    ...
    if primary_path.exists() and primary_path.is_dir():
        logger.info(f"Using shared folder: {primary_path}")
        return primary_path
    
    # Fallback to local directory
    fallback_path = Path(FALLBACK_JSON_FOLDER)
    logger.warning(f"Shared folder not accessible, using fallback: {fallback_path}")
