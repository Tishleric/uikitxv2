"""
Robust Trade Converter

This script performs a direct conversion of the net position streaming CSV 
into a single, clean trade ledger file.

Version 3 uses a line-by-line reading approach to ensure no rows are dropped,
addressing issues with pandas' CSV parsing.

It will:
1. Robustly read all 120 rows of trade data from 'net_position_streaming (4).csv'.
...
def convert_source_data_robustly():
    """
    Reads, transforms, and saves the trade data to a single output file,
    ensuring all rows are processed correctly by reading line-by-line.
    """
    if not SOURCE_CSV.exists():
        print(f"Error: Source file not found at {SOURCE_CSV}")
        return

    print(f"Reading source file line-by-line: {SOURCE_CSV}")
    
    trades = []
    with open(SOURCE_CSV, 'r', newline='') as infile:
