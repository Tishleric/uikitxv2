...
def normalize_product(product):
    """Normalize product names (e.g., '10YR T NOTE' -> '10YR TNOTE')."""
    return product.replace(' T NOTE', ' TNOTE').replace(' T BOND', ' TBOND')
...
def generate_broker_symbol(row):
    ...
    try:
        ...
        # NOTE: No strike included - follows pattern of other formats
        parts = [month, year, 'CBT', broker_product]
        if week_info:
            parts.append(week_info)
        # Strike removed - handled during translation like other formats
