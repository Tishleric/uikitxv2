...
- **data_service.py**: Concrete implementation of data service for ActantEOD scenario metrics. Handles loading data from JSON files, storing in SQLite, and providing filtered access to data for dashboard components. Implements the DataServiceProtocol interface following ABC_FIRST principle. **ENHANCED**: Added PM data integration with automatic schema migration to support dual-source architecture via `data_source` column. **ENHANCED**: Added `get_shock_values()` method and shock value filtering support in `get_filtered_data()` for granular shock amount selection. **ENHANCED**: Added `get_shock_values_by_type()` method for dynamic shock amount options based on shock type selection. **NEW**: Added risk metric transformations for bs_Delta, bs_Gamma, and bs_Vega using DV01 (0.063) with clear, reviewable formulas applied during data loading pipeline.

- **dashboard_eod.py**: Comprehensive redesigned ActantEOD dashboard with multi-scenario visualization, metric categorization (Delta, Epsilon, Gamma, Theta, Vega, Zeta, Vol, OEV, Th PnL, Misc), prefix filtering (ab_, bs_, pa_, base), dynamic grid layouts, per-scenario range sliders, and table/graph toggle views **NEW**: Added visualization mode toggle between "Scenario View" (each scenario = one graph with all metrics) and "Metric View" (each metric = one graph with all scenarios). Includes dual visualization modes with MATCH pattern callbacks for both scenario-based and metric-based components, global shock range calculation for metric mode, and responsive grid layouts adapting to selected items. **REDESIGNED**: Replaced Toggle components with modern button-based UI using horizontal layout - three button pairs (View: Graph/Table, Values: Absolute/Percentage, Mode: Scenario/Metric) with shared state store, dynamic styling, and visual feedback following dashboard.py design pattern.
